...
Windows часто кладёт zip в `...\Lipi-os-...(1)\...`. Раньше путь шёл через
`env LIPI_WSL_ROOT=...`, и bash воспринимал `(` как синтаксис.

Сейчас путь передаётся как аргумент `"$1"` — скобки и пробелы допустимы.
Можно и просто переименовать папку без `(1)`, но это уже не обязательно.
