...
### Если PowerShell пишет `The string is missing the terminator`

Старые версии вызывали сборку через `powershell -Command "..."` с вложенными кавычками — это ломалось.  
Сейчас `.bat` вызывает `iso\build-windows.ps1` через `-File` (без кавычечного ада). Обновите ветку и запустите снова.
